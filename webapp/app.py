...
@app.get("/healthz")
async def healthz():
    return {"status": "ok", "service": "flighttrackr", "push": push_configured()}
...
async def push_test(request: Request):
    ...
    payload = {
        "title": str(body.get("title") or "FlightTrackr"),
        "body": str(body.get("body") or "Push notifications are active."),
        "url": "/",
    }
    sent = 0
    for subscription in all_subscriptions():
        try:
            ...
            if is_gone(exc):
                remove(subscription.get("endpoint", ""))
...
async def flight_status(
    request: Request,
    flight_number: str = Query(..., min_length=2, max_length=12),
