...
from .live_enrichment_combined import enrich_live_aircraft
...
async def nearby(request: Request, lat:float=Query(...,ge=-90,le=90), lon:float=Query(...,ge=-180,le=180), radius:float=Query(25,gt=0,le=MAX_RADIUS_NM)):
    _rate_check(request); radius_nm=max(1,min(MAX_RADIUS_NM,radius))
    try:
        async with httpx.AsyncClient(timeout=8.0,follow_redirects=True) as client:
            response=await client.get(f"{ADSBLOL_BASE}/v2/point/{lat}/{lon}/{radius_nm}",headers={"Accept":"application/json","User-Agent":"FlightTrackr/1.0"}); response.raise_for_status(); data=response.json()
    ...
    for item in data.get("ac",[]):
        ...
        metadata=await enrich_live_aircraft(icao24=item.get("hex"),callsign=(item.get("flight") or "").strip() or None,registration=item.get("r"),base_operator=item.get("own") or item.get("owner"),base_aircraft_type=item.get("t"),base_description=item.get("desc"))
        aircraft.append({"hex":item.get("hex"),"callsign":(item.get("flight") or "").strip() or None,"registration":metadata.get("registration") or item.get("r"),"airline":metadata.get("airline") or metadata.get("operator"),"operator":metadata.get("operator") or metadata.get("airline"),"type":metadata.get("aircraft_type") or item.get("t"),"description":metadata.get("aircraft_description") or item.get("desc"),"origin":metadata.get("origin"),"destination":metadata.get("destination"),"route":metadata.get("route"),"metadata_source":metadata.get("metadata_source"),"lat":item.get("lat"),"lon":item.get("lon"),"altitude":item.get("alt_baro") if item.get("alt_baro") is not None else item.get("alt_geom"),"speed":item.get("gs"),"track":item.get("track"),"vertical_rate":item.get("baro_rate") if item.get("baro_rate") is not None else item.get("geom_rate"),"squawk":item.get("squawk"),"seen":item.get("seen")})
    return {"provider":"adsb.lol+opensky+pyopensky","retrieved_at":int(time.time()),"radius_nm":radius_nm,"aircraft":aircraft}
...
async def aircraft(hex_code:str,request:Request):
    ...
    try:
        async with httpx.AsyncClient(timeout=8.0,follow_redirects=True) as client:
            response=await client.get(f"{ADSBLOL_BASE}/v2/hex/{clean}",headers={"Accept":"application/json"}); response.raise_for_status(); data=response.json()
    except (httpx.HTTPError,ValueError) as exc: raise HTTPException(status_code=502,detail="Live aircraft provider unavailable.") from exc
    item=(data.get("ac") or [{}])[0]
    metadata=await enrich_live_aircraft(icao24=clean,callsign=(item.get("flight") or "").strip() or None,registration=item.get("r"),base_operator=item.get("own") or item.get("owner"),base_aircraft_type=item.get("t"),base_description=item.get("desc"))
    return {"provider":"adsb.lol+opensky+pyopensky","aircraft":item,"metadata":metadata}
