...
const freshnessLabel = (flight) => {
  if (flight.freshness === "fresh") return `● Frisch · ${flight.position_age_seconds ?? 0}s`;
  if (flight.freshness === "aging") return `● Alternd · ${flight.position_age_seconds ?? "–"}s`;
  return `● Veraltet · ${flight.position_age_seconds ?? "–"}s`;
};
...
function updateMapCenter() {
  ...
  const lat = position.coords.latitude, lon = position.coords.longitude, radiusMeters = Number($("radius").value) * 1000;
...
function aircraftIcon(flight) {
  ...
  return L.divIcon({ className: "aircraft-icon-wrap", html: `<div class="aircraft-icon freshness-${flight.freshness || "unknown"}" style="transform:rotate(${rotation}deg)">✈</div>`, iconSize: [34, 34], iconAnchor: [17, 17] });
}
...
function updateAircraftOnMap(flights) {
  ...
  for (const flight of flights) {
    ...
    const popup = `<strong>${flight.callsign || "Unbekannt"}</strong><br>${flight.distance_km.toFixed(2)} km · ${flight.altitude_ft ?? "–"} ft<br>${flight.velocity_kmh ?? "–"} km/h<br><small>${flight.source || "–"} · ${freshnessLabel(flight)} · Qualität ${flight.quality_score ?? "–"}</small>`;
    ...
    } else aircraftMarkers.set(key, L.marker(latLng, { icon: aircraftIcon(flight), keyboard: false }).addTo(map).bindPopup(popup));
  }
  for (const [key, marker] of aircraftMarkers) if (!active.has(key)) { map.removeLayer(marker); aircraftMarkers.delete(key); }
}
...
function renderFlights(flights) {
  ...
  if (!flights.length) { list.innerHTML = '<article class="empty">Aktuell wurde innerhalb deines Radius kein Flugzeug gemeldet.</article>'; return; }
  list.innerHTML = flights.map((flight) => `
    <article class="flight-card">
      ...
      <div class="data-quality"><span class="freshness ${flight.freshness || "unknown"}">${freshnessLabel(flight)}</span><span>${flight.source || "Unbekannt"}</span><span>Qualität ${flight.quality_score ?? "–"}</span></div>
...
function enableLocation() {
  ...
  navigator.geolocation.getCurrentPosition((result) => {
    position = result; $("locationCard").hidden = false;
    ...
    initMap(result.coords.latitude, result.coords.longitude); refresh(); startTimer();
